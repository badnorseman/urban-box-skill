...
const getFileType = fileFormat => {
  ...
  else if (boxDocumentFormats.includes(fileFormat))
...
function FilesReader(body) {
  ...
  this.fileDownloadURL = `${BOX_API_ENDPOINT}/files/${
    this.fileId
  }/content?access_token=${this.fileReadToken}`;
}
 ...
 */
FilesReader.prototype.validateFormat = function validateFormat(
  allowedFileFormatsList
) {
 ...
 */
FilesReader.prototype.validateSize = function validateSize(
  allowedMegabytesNum
) {
  const fileSizeMB = this.fileSize / MB_INTO_BYTES;
  if (fileSizeMB <= allowedMegabytesNum) return true;
  throw new Error(
    `File size ${fileSizeMB} MB is over accepted limit of ${allowedMegabytesNum} MB`
  );
};
...
FilesReader.prototype.getContentStream = function getContentStream() {
  return new Promise((resolve, reject) => {
    this.fileReadClient.files.getReadStream(
      this.fileId,
      null,
      (error, stream) => {
        if (error) {
...
FilesReader.prototype.getBasicFormatFileURL = function getBasicFormatFileURL() {
  ...
  return this.fileReadClient.files
    .getRepresentationInfo(
      this.fileId,
      FileType[this.fileType].representationType
    )
    .then(reps => {
    ...
    .then(assetURLTemplate => {
      return `${urlTemplate
        ...
        .expand({ asset_path: options.assetPath })}?access_token=${
        this.fileReadToken
      }`;
    });
};
...
SkillsWriter.prototype.error = {
  ...
  INVALID_FILE_SIZE: {
    ...
    message:
      "Something went wrong with processing the file. This file size is currently not supported."
  },
  INVALID_FILE_FORMAT: {
    code: "skills_invalid_file_format_error",
    message:
      "Something went wrong with processing the file. Invalid information received."
  },
  INVALID_EVENT: {
...
const validateUsage = usage =>
  ...
  usage.unit &&
  usage.value &&
  validateEnum(usage.unit, usageUnit) &&
...
const processCardData = (cardData, duration) => {
  if (!cardData.text)
    throw new TypeError(
      `Missing required 'text' field in ${JSON.stringify(cardData)}`
    );
  cardData.type = typeof cardData.image_url === "string" ? "image" : "text";
  if (
    duration &&
    !(Array.isArray(cardData.appears) && cardData.appears.length > 0)
  ) {
    console.warn(
...
const putData = (client, skillId, body, callback) => {
  ...
  console.debug("########################################"); // Remove when implemented successfully
  console.debug(`File id: ${body.file.id}`); // Remove when implemented successfully
  console.debug(`API path: ${apiPath}`); // Remove when implemented successfully
  console.debug("Params: ", params); // Remove when implemented successfully
  console.debug("Session info: ", client._session); // Remove when implemented successfully
  console.debug("########################################"); // Remove when implemented successfully
...
SkillsWriter.prototype.createTranscriptsCard = function createTranscriptsCard(
  ...
  transcriptsDataList.forEach(transcript =>
    processCardData(transcript, optionalFileDuration)
  );
  return this.createMetadataCard(
    cardType.TRANSCRIPT,
...
SkillsWriter.prototype.createFacesCard = function createFacesCard(
  ...
  return new Promise((resolve, reject) => {
    Promise.all(dataURIPromises)
      .then(dataURIs => {
        for (let i = 0; i < facesDataList.length; i++) {
          facesDataList[i].image_url =
            dataURIs[i] || facesDataList[i].image_url;
        }
        resolve(cards);
      })
      .catch(error => {
 ...
 * card has been saved.
 */
SkillsWriter.prototype.saveProcessingCard = function saveProcessingCard(
  optionalCallback
) {
  const status = {
  ...
  const statusCard = this.createMetadataCard(
    cardType.STATUS,
    cardTitle.STATUS,
    status
  );
  return this.saveDataCards(
    [statusCard],
    optionalCallback,
    skillInvocationStatus.PROCESSING
  );
};
...
SkillsWriter.prototype.saveErrorCard = function saveErrorCard(
  ...
  const errorJSON = validateEnum(error, this.error)
    ? error
    : this.error.UNKNOWN;
  const errorCard = this.createMetadataCard(
    cardType.STATUS,
    cardTitle.ERROR,
    errorJSON
  );
...
SkillsWriter.prototype.saveDataCards = function saveDataCards(
  ...
  const status = validateEnum(optionalStatus, skillInvocationStatus)
    ? optionalStatus
    : skillInvocationStatus.SUCCESS;
